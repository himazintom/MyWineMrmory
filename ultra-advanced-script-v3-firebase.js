...
// フォーム入力の一時保存機能（localStorageは前回入力内容の保存のみに使用）
...
function setupEventListeners() {
    ...
    if (privacyToggleBtn) {
        privacyToggleBtn.addEventListener('click', togglePrivacySetting);
    }
...
async function loadPrivacySetting() {
    try {
        ...
        updatePrivacyUI(privacy.isPublic);
    } catch (error) {
        console.error('❌ プライバシー設定読み込みエラー:', error);
        updatePrivacyUI(false); // デフォルトは非公開
    }
}
...
async function togglePrivacySetting() {
    ...
    try {
        const currentPrivacy = await getUserPrivacy();
        const newIsPublic = !currentPrivacy.isPublic;
        
        let shareUrl = currentPrivacy.shareUrl;
        
        if (newIsPublic && !shareUrl) {
            // 公開にする場合は共有URLを生成
            shareUrl = generateShareUrl(currentUser.uid);
        }
        ...
        await updateUserPrivacy(newPrivacySettings);
        updatePrivacyUI(newIsPublic);
        
        if (newIsPublic) {
            // 共有URLをクリップボードにコピー
            await copyToClipboard(shareUrl);
            showNotification('記録を公開にしました。共有URLをクリップボードにコピーしました。', 'success');
        } else {
            showNotification('記録を非公開にしました。', 'info');
        }
        
    } catch (error) {
        console.error('❌ プライバシー設定切り替えエラー:', error);
        showNotification('設定の変更に失敗しました', 'error');
    }
}
...
function updatePrivacyUI(isPublic) {
    ...
    const privacyIcon = privacyBtn.querySelector('.privacy-icon');
    const privacyText = privacyBtn.querySelector('.privacy-text');
    
    if (isPublic) {
        privacyBtn.className = 'privacy-btn public';
        privacyIcon.textContent = '🌐';
        privacyText.textContent = '公開';
    } else {
        privacyBtn.className = 'privacy-btn private';
        privacyIcon.textContent = '🔒';
